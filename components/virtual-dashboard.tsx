...
function LiveDot({ pulseRef }: { pulseRef: React.RefObject<HTMLSpanElement | null> }) {
  return (
...
export function VirtualDashboard() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.set([headerRef.current, leftColRef.current, rightColRef.current], { opacity: 0, y: 20 })
      gsap.set(kpiRefs.current, { opacity: 0, y: 16 })

      const tl = gsap.timeline({ defaults: { ease: "power3.out" } })
      tl.to(headerRef.current,  { opacity: 1, y: 0, duration: 0.45 })
        .to(kpiRefs.current,    { opacity: 1, y: 0, stagger: 0.08, duration: 0.5 }, "-=0.2")
        .to(leftColRef.current, { opacity: 1, y: 0, duration: 0.5 }, "-=0.3")
        .to(rightColRef.current,{ opacity: 1, y: 0, duration: 0.5 }, "-=0.35")

      tl.call(() => {
        CHANNELS.forEach((ch, i) => {
          const el = barRefs.current[i]
          if (el) gsap.to(el, { width: `${ch.pct}%`, duration: 1, ease: "power2.out", delay: i * 0.08 })
        })
        ...
        keys.forEach((k, i) => {
          const obj = { val: 0 }
          gsap.to(obj, {
            val: targets[i], duration: 1.2, ease: "power2.out",
  ...
  const cardLift = {
    rest:  { y: 0,  boxShadow: "0 1px 3px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.05)" },
    hover: { y: -3, boxShadow: "0 8px 30px rgba(0,0,0,0.10), 0 0 0 1px rgba(0,0,0,0.06)", transition: { type: "spring" as const, stiffness: 280, damping: 22 } },
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: "0 4px 6px -1px rgba(0,0,0,0.07), 0 2px 4px -2px rgba(0,0,0,0.05), 0 0 0 1px rgba(0,0,0,0.04)",
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <LiveDot pulseRef={pulseRef} />
          {/* period selector */}
          <div className="flex items-center gap-px rounded-lg overflow-hidden"
            style={{ border: "1px solid #e2e8f0" }}>
            {["1D", "7D", "30D"].map((p, i) => (
              <button key={p} className="px-2.5 py-1 text-[10px] font-semibold transition-colors cursor-pointer"
                style={{
                  background: i === 0 ? "#2664eb" : "#ffffff",
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-px"
        ...
        {KPI.map(({ label, Icon, accent, bg, delta }, i) => (
          <motion.div
            ...
            className="flex items-start gap-3 px-5 py-4 cursor-default"
          ...
          >
            ...
            <div className="min-w-0">
              <div className="text-[20px] font-bold text-slate-900 leading-none mb-0.5" style={{ fontVariantNumeric: "tabular-nums" }}>
                {fmtCounter(["numbers", "calls", "success", "uptime"][i] as keyof typeof counters)}
              </div>
              <div className="text-[11px] text-slate-500">{label}</div>
              <div className="text-[10px] font-semibold mt-1" style={{ color: accent }}>{delta}</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px]"
        ...
        >
          {/* Call volume chart */}
          <div className="flex-1 flex flex-col px-5 py-4" style={{ borderBottom: "1px solid #f1f5f9" }}>
            <div className="flex items-center justify-between mb-3">
              <div>
                <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-0.5">Call Volume</div>
                <div className="text-[13px] font-semibold text-slate-800">Today · 24-hour view</div>
              </div>
              <div className="flex items-center gap-3">
                {[["#2664eb", "Total calls"], ["#0891b2", "Connected"]].map(([c, l]) => (
          ...
          <div className="px-5 py-4" style={{ background: "#ffffff" }}>
            <div className="flex items-center justify-between mb-3">
              <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest">Recent Calls</div>
              <button className="text-[11px] font-semibold cursor-pointer" style={{ color: "#2664eb" }}>
                ...
                <div key={i} className="flex items-center justify-between py-2.5">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center"
                      style={{ background: STATUS_CONFIG[call.status as keyof typeof STATUS_CONFIG].bg }}>
                      <Phone size={11} color={STATUS_CONFIG[call.status as keyof typeof STATUS_CONFIG].color} strokeWidth={2} />
                    </div>
                    <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0 ml-3">
                    <span className="text-[11px] text-slate-400 font-mono">{call.dur}</span>
                    <StatusPill status={call.status as keyof typeof STATUS_CONFIG} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* RIGHT — numbers list + channels ──────────────────── */}
        <div ref={rightColRef} className="flex flex-col" style={{ background: "#ffffff" }}>

          {/* Active numbers list */}
          <div className="px-5 py-4 flex-1" style={{ borderBottom: "1px solid #e2e8f0" }}>
            <div className="flex items-center justify-between mb-3">
              <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest">Active Numbers</div>
            ...
            <div className="space-y-1">
              {NUMBERS.map((n) => (
                ...
                >
                  {/* region badge */}
                  <div className="w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center text-[9px] font-black"
                    style={{
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[11.5px] font-semibold text-slate-800 truncate">{n.number}</div>
          ...
          <div className="px-5 py-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest">Channel Usage</div>
                ...
                <div key={ch.label}>
                  ...
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "#f1f5f9" }}>
                    <div
                      ...
                      style={{ width: "0%", background: ch.accent, transition: "width 0.1s" }}
                    />
                  </div>
                </div>
              ))}
